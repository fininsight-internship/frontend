...
import { ChevronLeft, CheckCircle, Copy, RefreshCw, Star, Loader2 } from 'lucide-react';
...
export default function ResumeEditorPage() {
  ...
  const completedCount = coverQuestions.filter((_, i) => drafts[i]?.trim()).length;
  const totalProgress = coverQuestions.length > 0
    ? Math.round((completedCount / coverQuestions.length) * 100)
    : 0;
    ...
    const run = async () => {
      ...
      try {
        const res = await api.post('/resume/evaluate-detailed', {
          ...
          job_title: jobTitle,
          cover_question: coverQuestions[0],
          selections,
  ...
  const handleEvaluate = async () => {
    ...
    try {
      const res = await api.post('/resume/evaluate-detailed', {
        ...
        company_name: companyName,
        job_title: jobTitle,
        cover_question: coverQuestions[currentIdx],
        selections,
        company_insights: companyInsights,
      });
      setEvaluations((prev) => ({
      ...
      <div className={styles.body}>
        ...
        <aside className={styles.sidebar}>
          ...
          <div className={styles.progressSection}>
            ...
            <p className={styles.progressSub}>{completedCount} / {coverQuestions.length} 문항 완료</p>
          </div>
        </aside>
        ...
        <main className={styles.main}>
          ...
          <div className={styles.editorCard}>
            <div className={styles.editorTop}>
              <div className={styles.questionLabel}>
                ...
                <span className={styles.questionFull}>{coverQuestions[currentIdx]}</span>
              </div>
          ...
          {(evaluating || currentEval) && (
            <div className={`${styles.evalCard} ${evaluating ? styles.evalCardLoading : ''}`}>
